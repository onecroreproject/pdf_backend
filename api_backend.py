...
async def download_youtube(
    ...
    try:
        ...
        ydl_opts = {
            ...
            'quiet': True,
            'cookiefile' : r"R:\dinesh\tools\PDF Creator\PDF-convertor\pdf_tools\youtube.com_cookies.txt"
        }
...
async def download_instagram(url: str = Form(...)):
    ...
    try:
        ydl_opts = {
            ...
            'outtmpl': outtmpl,
            'quiet': True,
            'cookiefile': r"R:\dinesh\tools\PDF Creator\PDF-convertor\pdf_tools\instagram.com_cookies",  # ✅ Adjust this path if needed
            'noplaylist': True,
        }
...
def parse_equation(equation: str):
    """
    Parses full chemical equation into reactants and products.
    Accepts formats like: "H2 + O2 = H2O" or "Fe + O2 -> Fe2O3"
    """
    if "=" in equation:
    ...
    if not reactants or not products:
        raise ValueError("Could not parse reactants or products")

    return reactants, products


def make_equation_string(coeffs_reac, coeffs_prod, reac_list, prod_list):
    """
    Construct the balanced chemical equation string.
    Only include coefficients > 1.
    """
    def term(coeff, formula):
        return f"{coeff if coeff != 1 else ''}{formula}"

    left = " + ".join(term(coeffs_reac[f], f) for f in reac_list)
    right = " + ".join(term(coeffs_prod[f], f) for f in prod_list)
    return f"{left} = {right}"

# --- API Route ---

@app.post("/chemical/balance")
async def balance_chemical_equation(equation: str = Form(...)):
    """
    Accepts: equation = "CH4 + O2 = CO2 + H2O"
    Returns: Balanced equation string
    """
    try:
        reac_list, prod_list = parse_equation(equation)

        # Balance using chempy
        reac_coeffs, prod_coeffs = balance_stoichiometry(set(reac_list), set(prod_list))

        # Convert back to full equation string
        balanced = make_equation_string(reac_coeffs, prod_coeffs, reac_list, prod_list)
...
# Predefined lists for fallback
openings = ["Set the scene", "Launch into action", "Focus on a relationship"]
conflicts = ["emotional", "violent", "financial"]
resolutions = ["happy", "sad", "violent"]
places = ["village", "forest", "city", "desert", "mountain"]
weathers = ["sunny", "stormy", "foggy", "rainy", "snowy"]
occasions = ["birthday", "wedding", "funeral", "holiday", "festival"]
verbs = ["running", "searching", "whispering", "fighting", "healing"]
vulnerables = ["old man", "baby bird", "child", "lost dog", "stray cat"]
comparisons = ["angel", "giant", "monster", "saint", "storm"]
character_traits = ["brave", "timid", "curious", "grumpy", "kind"]
titles = ["The Unexpected Journey", "Echoes of Fate", "Shadows of Tomorrow", "Whispers in the Wind", "Broken Promises"]
names = ["Alex", "Riley", "Jordan", "Morgan", "Casey", "Taylor", "Jamie", "Sam"]

# Helper to pick random if input is empty or "Random"
def pick_random(value, choices):
    if value and value.strip() and value.lower() != "random":
        return value.strip()
...
async def generate_custom_story(
    title: str = Form(None),
    name_protagonist: str = Form(None),
    name_secondary: str = Form(None),
    opening_style: str = Form(None),
    conflict_type: str = Form(None),
    resolution_type: str = Form(None),
    secondary_want: str = Form(None),
    secondary_feeling: str = Form(None),
    protagonist_says: str = Form(None),
    place_name: str = Form(None),
    place_feeling: str = Form(None),
    place_feature: str = Form(None),
    place_adjective1: str = Form(None),
    place_adjective2: str = Form(None),
    char_adj1: str = Form(None),
    char_adj2: str = Form(None),
    occasion: str = Form(None),
    verb1: str = Form(None),
    verb2: str = Form(None),
    vulnerable: str = Form(None),
    compare_pro: str = Form(None),
    compare_sec: str = Form(None),
):
    try:
        # Fill in blanks with random values if not provided
        title = pick_random(title, titles)
        name_pro = pick_random(name_protagonist, names)
        name_sec = pick_random(name_secondary, names)
        opening_style = pick_random(opening_style, openings)
        conflict_type = pick_random(conflict_type, conflicts)
        resolution_type = pick_random(resolution_type, resolutions)
        secondary_want = pick_random(secondary_want, ["closure", "revenge", "a hug", "forgiveness", "an answer"])
        secondary_feeling = pick_random(secondary_feeling, ["positively", "negatively"])
        protagonist_says = pick_random(protagonist_says, ["I love you", "I'm sorry", "I need to go", "Don't leave me"])
        place_name = pick_random(place_name, places)
        place_feeling = pick_random(place_feeling, ["hopeful", "lonely", "peaceful", "ominous"])
        place_feature = pick_random(place_feature, ["trees", "rivers", "mountains", "houses", "fog"])
        place_adj1 = pick_random(place_adjective1, ["quiet", "eerie", "bustling", "serene"])
        place_adj2 = pick_random(place_adjective2, ["ancient", "modern", "haunted", "peaceful"])
        char_adj1 = pick_random(char_adj1, character_traits)
        char_adj2 = pick_random(char_adj2, character_traits)
        occasion = pick_random(occasion, occasions)
        verb1 = pick_random(verb1, verbs)
        verb2 = pick_random(verb2, verbs)
        vulnerable = pick_random(vulnerable, vulnerables)
        compare_pro = pick_random(compare_pro, comparisons)
        compare_sec = pick_random(compare_sec, comparisons)

        # Compose the story using a simple template
        story = f"**{title}**\n\n"
        story += f"{opening_style}: In the {place_adj1} town of {place_name}, known for its {place_feature}, "
        story += f"lived {name_pro}, a {char_adj1} soul who often felt {place_feeling}. "
        story += f"One day, {name_sec}, a {char_adj2} character from the past, returned unexpectedly.\n\n"
        story += f"{name_sec} wanted {secondary_want} from {name_pro}, and felt {secondary_feeling} about them. "
        story += f"{name_pro} whispered, “{protagonist_says}.”\n\n"
        story += f"But {conflict_type} tension quickly rose. {name_pro} found themselves {verb1}, while {name_sec} was {verb2}. "
        story += f"In the middle of it all, a {vulnerable} crossed their path — changing everything.\n\n"
        story += f"A witness said {name_pro} looked like a {compare_pro}, while {name_sec} seemed like a {compare_sec}.\n\n"
        story += f"Finally, at the {occasion}, a {resolution_type} ending brought the story to its close.\n"

        return JSONResponse({
            "title": title,
            "protagonist": name_pro,
            "secondary_character": name_sec,
            "story": story
        })
...
###
first_names_male = [
    "Liam", "Noah", "James", "William", "Benjamin", "Lucas", "Henry", "Alexander", "Ethan", "Jacob",
    "Michael", "Daniel", "Matthew", "Joseph", "Samuel", "David", "Jackson", "Sebastian", "Aiden", "Owen",
    "Gabriel", "Carter", "Wyatt", "John", "Jack", "Luke", "Jayden", "Dylan", "Grayson", "Levi",
    "Isaac", "Julian", "Mateo", "Ryan", "Jaxon", "Nathan", "Caleb", "Hunter", "Thomas", "Aaron",
    "Charles", "Connor", "Eli", "Ezra", "Colton", "Adrian", "Jonathan", "Nolan", "Jeremiah", "Easton",
    "Elias", "Christopher", "Miles", "Jaxson", "Robert", "Hudson", "Lincoln", "Angel", "Brayden", "Jordan",
    "Dominic", "Austin", "Ian", "Adam", "Ezekiel", "Jose", "Carson", "Xavier", "Leonardo", "Jake",
    "Luca", "Asher", "Kayden", "Ryder", "Gavin", "Bentley", "Blake", "Nathaniel", "Avery", "Steven",
    "Eric", "Santiago", "Damian", "Bryson", "Parker", "Weston", "Jason", "Elias", "Sawyer", "Micah",
    "Vincent", "Rylan", "Cole", "Hayden", "George", "Ivan", "Max", "Zachary", "Axel", "Justin"
]
...
first_names_female = [
    ...
    "Natalie", "Naomi", "Eliana", "Brooklyn", "Elena", "Aaliyah", "Savannah", "Claire", "Audrey", "Skylar",
    "Genesis", "Bella", "Aaliyah", "Kennedy", "Samantha", "Maya", "Sarah", "Madelyn", "Adeline", "Alexa",
    "Ariana", "Allison", "Hailey", "Gabriella", "Alice", "Madeline", "Cora", "Ruby", "Eva", "Serenity",
    "Autumn", "Piper", "Sophie", "Quinn", "Nevaeh", "Caroline", "Ivy", "Sadie", "Natalia", "Josephine",
    "Delilah", "Jade", "Peyton", "Lydia", "Brielle", "Adalynn", "Clara", "Vivian", "Reagan", "Mackenzie"
]
...
@app.get("/name/random")
async def generate_random_names(
    count: int = Query(default=1, ge=1, le=1000, description="Number of names to generate (1-1000)"),
    gender: Gender = Gender.any,
    name_type: NameType = NameType.full,
):
    """
    Generate random names.

    - **count**: Number of names to generate (1 to 1000)
    - **gender**: 'male', 'female', or 'any' (affects first names)
    - **name_type**: 'first', 'last', or 'full'
    """
    
    # Prepare first name pool based on gender
    ...
    for _ in range(count):
        if name_type == NameType.first:
            results.append(random.choice(first_names))
        elif name_type == NameType.last:
